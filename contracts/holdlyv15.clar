...
;; Contract Address
(define-constant CONTRACT_ADDRESS 'SP3N8PR8ARF68BC45EDK4MWZ3WWDM74CFJB3SS99R.holdlyv15)
...
;; tracks how many history entries each user has
...
(define-private (send-stx-from-contract (amount uint) (recipient principal))
    (as-contract
        (stx-transfer? amount contract-caller recipient)
    )
)
...
;; allow rating after successful return
...
;; store history entry permanently
(let (
    (history-index
        (+ 
            (default-to u0
                (map-get? user-history-count tx-sender)
            )
            u1
        )
    )
...
(define-read-only (get-user-history-count (user principal))
    (ok
        (default-to u0
            (map-get? user-history-count user)
        )
    )
)
