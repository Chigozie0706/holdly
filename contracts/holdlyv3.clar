...
;; Contract Address
(define-constant CONTRACT_ADDRESS 'ST3N8PR8ARF68BC45EDK4MWZ3WWDM74CFJAGZBY3K.holdlyv3)
...
(define-public (borrow-book (book-id uint))
    (let ((book (unwrap! (map-get? books book-id) ERR_BOOK_NOT_FOUND)))
        ...
        (unwrap!
            (contract-call? SBTC_TOKEN transfer (get deposit-amount book)
                tx-sender CONTRACT_ADDRESS none
            )
            ERR_TRANSFER_FAILED
        )
...
(define-public (return-book (book-id uint))
    (let (
        ...
        ;; Return sBTC from contract to borrower
        (unwrap!
            (as-contract (contract-call? SBTC_TOKEN transfer (get deposit-amount borrow)
                tx-sender (get borrower borrow) none
            ))
            ERR_TRANSFER_FAILED
        )
